...
# calculate_loss() modified to calculate Bending Loss
...
class CellViTTrainer(BaseTrainer):
    ...
    def train_step(
        ...
        #print(masks["nuclei_type_map"].shape)
        ...
        if self.mixed_precision:
            with torch.autocast(device_type="cuda", dtype=torch.float16):
                ...
                # calculate loss
                total_loss = self.calculate_loss(predictions, gt, imgs)
    ...
    def validation_step(
        ...
        if self.mixed_precision:
            with torch.autocast(device_type="cuda", dtype=torch.float16):
                ...
                gt = self.unpack_masks(masks=masks, tissue_types=tissue_types)
                # calculate loss
                _ = self.calculate_loss(predictions, gt,imgs)
    ...
    def unpack_masks(self, masks: dict, tissue_types: list) -> DataclassHVStorage:
        ...
        #mask = (nuclei_type_maps < 0) | (nuclei_type_maps > 5)
        #nuclei_type_maps[mask] = 0
        #print(nuclei_type_maps.max())
        #if nuclei_type_maps.max()>5:
            #print(nuclei_type_maps)
        #    for m in nuclei_type_maps:
        #        for r in range(256):
        #            for c in range(256):
        #                if m[r][c].item()>5:
        #                    print(f"r = {r}, c = {c}")
    ...
    def calculate_loss(
        self, predictions: DataclassHVStorage, gt: DataclassHVStorage, imgs
    ) -> torch.Tensor:
        """Calculate the loss
        ...
        for branch, pred in predictions.items():
            ...
            for loss_name, loss_setting in branch_loss_fns.items():
                # Debug: Printing loss function name
                ...
                #print(f"Ground Truth: {gt[branch].shape}")
                if loss_name == "msge":
                    loss_value = loss_fn(
                        input=pred,
                        target=gt[branch],
                ...
                elif loss_name == "samsloss":
                    loss_value = loss_fn(
                        input=[pred,imgs],
                        target=gt[branch],
                    )
                    #sams_loss += loss_value
                else:
        ...
        #loss_fn = retrieve_loss_fn("BendingLoss",alpha=1)
        #regularization_value,alpha = loss_fn(input=predictions["instance_types"])
        #regularization_value,alpha = loss_fn(input=predictions)
        #print((alpha * regularization_value).device)
        #total_loss = total_loss + (alpha * regularization_value)
