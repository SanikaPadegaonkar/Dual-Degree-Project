...
sys.path.append("/home/ravi/sanika/CellViT_with_puma/")
...
class PumaDataset(CellDataset):
    """PUMA dataset

    Args:
        dataset_path (Union[Path, str]): Path to PUMA dataset. Structure is described under ./docs/readmes/cell_segmentation.md
    ...
    def __init__(
        ...
        for fold in folds:
            ...
            #print(f"Printing self.types: {self.types}")

        logger.info(f"Created PUMA Dataset by using fold(s) {self.folds}")
    ...
    def __getitem__(self, index: int) -> Tuple[torch.Tensor, dict, str, str]:
        ...
        if self.transforms is not None:
            #print(f"Printing img shape: {img.shape}")
            #print(f"Printing mask shape: {mask.shape}")
        ...
        np_map = mask[:, :, 0].copy()
        np_map[np_map > 0] = 1
        hv_map = PumaDataset.gen_instance_hv_map(inst_map)
        ...
        if self.stardist:
            dist_map = PumaDataset.gen_distance_prob_maps(inst_map)
            stardist_map = PumaDataset.gen_stardist_maps(inst_map)
            masks["dist_map"] = torch.Tensor(dist_map).type(torch.float32)
            masks["stardist_map"] = torch.Tensor(stardist_map).type(torch.float32)
        if self.regression:
            masks["regression_map"] = PumaDataset.gen_regression_map(inst_map)
    ...
    def get_sampling_weights_cell(self, gamma: float = 1) -> torch.Tensor:
        ...
        #binary_weight_factors = np.array([4191, 4132, 6140, 232, 1528]) # for pannuke
        binary_weight_factors = np.array([1795,1694,2207,7165,21621,695,358,520,3843,57132]) # for puma: counts of all cell types
