...
class ExperimentCellVitPuma(BaseExperiment):
    ...
    def get_transforms(
        ...
        if "Affine".lower() in transform_settings:
            p = transform_settings["affine"]["p"]
            shear = transform_settings["affine"]["shear"]
            if p > 0 and p <= 1:
                transform_list.append(
                    A.Affine(p=p, shear=shear)
                )
        if "RandomBrightnessContrast".lower() in transform_settings:
            p = transform_settings["randombrightnesscontrast"]["p"]
            brightness_limit = transform_settings["randombrightnesscontrast"]["brightness_limit"]
            contrast_limit = transform_settings["randombrightnesscontrast"]["contrast_limit"]
            if p > 0 and p <= 1:
                transform_list.append(
                    A.RandomBrightnessContrast(p=p, brightness_limit=brightness_limit, contrast_limit=contrast_limit)
                )
        # ---------------------------------------------------------------------------------
