...
class FusionClassifier(nn.Module):
    def __init__(self, num_classes=16, hierarchy_map=None, pretrained=True):
        super().__init__()
...
class SegmentationDataset(Dataset):
    def __init__(self, image_paths, dims,context_scale = 2, is_train=True):
        ...
        for ip in self.image_paths:
            ...
            if os.path.exists(box_path) and os.path.exists(nuclei_path) and os.path.exists(class_path):
                self.data_info.append((ip, box_path, nuclei_path, class_path))
    ...
    def __getitem__(self, idx):
        ip, box_path, nuclei_path, class_path = random.choice(self.data_info)
...
def main():
    ...
    # Set up logs
    logs_dir = Path("logs_attention_context")
